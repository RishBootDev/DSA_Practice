public class OneZeroMatrix {
    ...
    // this solved the problem by complexity of O(m*n) but still not
    // optimized for the question
    // Best approach is Multi Source BFS
        Integer dp [][];
        ...
        public int[][] updateMatrix(int[][] mat) {
            ...
            for (int i = 0; i < mat.length; i++) {
                for (int j = 0; j < mat[0].length; j++) {
                    ...
                    dp = new Integer[mat.length][mat[0].length];
        ...
        public int helper(int[][] mat, int i, int j, boolean[][] vis) {
            ...
            vis[i][j] = false;
            return dp[i][j] = Math.min(up, Math.min(left, Math.min(right, down))) + 1;
        }
